...
def main():
    ...
    st.dataframe(
        df[['Student_ID','Total_Score','life_satisfaction',
            'closest_friend_count','disrespected_by_peers','fitness']].head(10)
    )
    ...
    if st.sidebar.button("Run Allocation"):
        ...
        st.bar_chart(
            df.groupby('Assigned_Class')['disrespected_by_peers']
              .apply(lambda x: (x=='Yes').mean()*100)
        )

        st.subheader("Detailed Assignments")
        st.dataframe(
            df[['Student_ID','Assigned_Class','fitness','Total_Score',
                'life_satisfaction','closest_friend_count','disrespected_by_peers']]
        )
